...
def get_mnoc(record_dict,ids=None,iou_targets=[0.85]):
    ...
    for iou_target in iou_targets:
        nocs=[]
        for id in ids:
            ious=np.array(record_dict[id]['ious'])      # ious是预测框和真实框之间的交并比(IOU)值列表。
            nocs.append(np.argmax(ious>=iou_target) if (ious>=iou_target).any() else (len(ious)-1))     # nocs是样本中所有预测框中与真实框交并比大于目标IOU值的最大IOU值所在位置的列表。np.argmax()函数返回符合条件的最大元素所在的位置，如果不存在符合条件的元素，则返回最后一个元素的位置。
